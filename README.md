...
- 每个语法点保留约 200 道题库储备，学习时按 40 题一轮推进
- 练习优先抽取未做题；核心首轮完成后通过错题和随机复习巩固
...
2. 使用 `createCuratedQuestionBank(grammarId, seeds, 200)` 建立题库储备
...
- Be 动词：20 个核心考点种子题，扩展为 200 题容量，学习时按 40 题一轮推进
- 一般现在时：20 个核心考点种子题，扩展为 200 题容量，学习时按 40 题一轮推进

其他语法点暂时使用 fallback 占位题库。后续应按语法顺序逐个替换为人工精选题库。

## 学习节奏

- 题库容量不是单次任务量。每个小节默认只推进 40 题核心首轮。
- 掌握线为核心首轮正确率不低于 85%，并优先清理错题。
- 错题需要连续答对 2 次才会标记为已掌握。
- 随机复习每次 10 题，用于阶段回顾和补弱。

## 后续扩展建议

- 继续补齐 `现在进行时`、`一般过去时`、`现在完成时` 等题库。
- 将题库从“种子题 + 变体扩展”逐步升级成真正 200 道完全独立题，但保持 40 题一轮的学习节奏。
