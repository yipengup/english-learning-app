...
function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {!practice ? <LearningView grammar={active} progress={progress} curated={curated} onStart={startPractice}/> :
...
function LearningView({ grammar, progress, curated, onStart }) {
  return <section className="content">
    <div className="hero"><div><span className="tag">{grammar.level}</span><h2>{grammar.title}</h2><p>{grammar.summary}</p><p className="quality"><Layers3 size={16}/>题库状态：{curated ? '已接入人工精选题库 + 变体扩展' : '暂用占位题库，待人工题库接入'}</p></div><div className="stats"><b>{progress.done}/{progress.total}</b><small>已练习</small><b>{progress.wrong}</b><small>错题</small><b>{progress.completionRate}%</b><small>完成度</small></div></div>
